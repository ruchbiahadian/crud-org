<template>
  <div class="container">
    ...
    <b-table :data="organizationAsset" striped bordered hoverable narrowed>
      <div>
      ...
      <b-table-column label="Aksi" centered header-class="table_header">
        <template v-slot="props">
          <b-dropdown>
            ...
            <b-dropdown-item @click="editOrgAsset(props.row._id)"
              >Rincian
            </b-dropdown-item>
            <b-dropdown-item
...
export default {
  ...
  methods: {
    async getOrganizationAsset() {
      ...
      try {
        await this.$axios.get("organizations-assets").then((resp) => {
    ...
    async deleteOrgAsset(id) {
      ...
      if (result.isConfirmed) {
        this.property.animation.isLoading = true;
        try {
          await this.$axios.delete("organizations-assets/" + id).then(() => {
            this.$store.dispatch("showSuccess", {
              text: "Aset organisasi berhasil dihapus!",
              router: this.$router,
            });
          });
        } catch (error) {
